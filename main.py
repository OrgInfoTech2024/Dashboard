...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.updatingProgressBar = QProgressBar(self)
        self.updatingProgressBar.setGeometry(150, 175, 300, 30)
        self.updatingProgressBar.hide()
    ...
    def updatePackage(self):
        try:
            ...
            process = subprocess.Popen(["sudo", "apt", "update", "&&", "sudo", "apt", "upgrade", "-y", "&&", "sudo", "apt-get", "update", "&&", "sudo", "apt-get", "upgrade", "-y"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
            ...
            while True:
                ...
                if output == '' and process.poll() is not None:
                    break
                if output:
                    self.updatingProgressBar.setValue(self.updatingProgressBar.value() + 1)

            rc = process.poll()
            if rc == 0:
                self.updatingProgressBar.setValue(100)
            else:
                self.updatingProgressBar.setValue(0)
    ...
    def downloadsUpdates(self):
        url = "https://github.com/OrgInfoTech2024/Dashboard/blob/main/update/updater.sh"
        file_path = "datas/updater.sh"
        ...
        process = subprocess.Popen(["bash", file_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
